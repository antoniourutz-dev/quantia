  ...
  return (
    <div className="bg-white p-12 rounded-[3rem] border border-slate-100 shadow-premium flex flex-col md:flex-row gap-16 items-center relative overflow-hidden group">
        ...
        <ResponsiveContainer width="100%" height="100%" className="relative z-10">
          <BarChart data={data}>
            ...
            <Bar 
              ...
              radius={[10, 10, 10, 10]} 
              barSize={36}
              className="drop-shadow-[0_10px_15px_rgba(79,70,229,0.2)]"
            >
              {data.map((_, index) => (
                <Cell 
                  key={`cell-${index}`} 
                  fill={index === new Date().getDay() - 1 ? '#4f46e5' : '#e2e8f0'} 
      ...
      <div className="flex-1 space-y-8 max-w-lg">
        <div className="inline-flex p-5 bg-indigo-50 rounded-[2rem] text-indigo-600 shadow-sm shadow-indigo-100 group-hover:rotate-12 transition-transform duration-700">
          <Lightbulb size={36} className="fill-indigo-600/10" />
        </div>
        
        <div className="space-y-4">
          <h3 className="text-4xl font-black text-slate-900 tracking-tighter">Análisis Semanal</h3>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-4 text-emerald-600 font-black text-lg bg-emerald-50 px-8 py-4 rounded-[2rem] w-fit border border-emerald-100 shadow-sm animate-in zoom-in duration-700">
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] pl-2">
            Algoritmo de rendimiento activo
          </p>
        </div>
      </div>
    </div>
