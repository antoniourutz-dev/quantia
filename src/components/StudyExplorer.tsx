...
export default function StudyExplorer({ bundle, onStartStudy }: StudyExplorerProps) {
  ...
  const availableTopics = useMemo(() => {
    ...
    deduped.sort((a, b) => a.localeCompare(b, 'es'));
    return deduped;
  }, [bundle, scope]);
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'No se ha podido iniciar el estudio.');
      setLoading(false);
    }
  };
      ...
      <div className="flex items-start justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 text-slate-400">
            <BookOpen size={18} />
            <span className="text-[10px] font-black uppercase tracking-[0.3em]">Estudio</span>
          </div>
          <h2 className="mt-3 text-3xl md:text-4xl font-black text-slate-900 tracking-tight">
            Estudia como un test
          </h2>
          <p className="mt-2 text-slate-500 text-lg font-medium">
            Elige temario y tema. Avanza pregunta a pregunta, viendo la respuesta correcta y la explicación.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 space-y-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-slate-700">
              ...
              <span className="font-black">Configuración</span>
            </div>
          </div>

          <div className="space-y-3">
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Temario</div>
            <div className="grid grid-cols-3 gap-2">
              {(['all', 'common', 'specific'] as StudyScope[]).map((value) => (
                ...
                >
                  {value === 'all' ? 'Todos' : value === 'common' ? 'Común' : 'Espec.'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Tema</div>
          ...
          <div className="rounded-2xl border border-slate-100 bg-slate-50 px-5 py-4">
            <div className="flex items-center gap-2 text-slate-600">
              ...
              <span className="text-xs font-black uppercase tracking-widest">Nota</span>
            </div>
            <div className="mt-2 text-sm font-medium text-slate-600 leading-relaxed">
              En modo Estudio no se sincroniza la sesión en estadísticas. Es lectura activa, no evaluación.
            </div>
          </div>
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8">
          <div className="flex items-center gap-3 text-slate-700">
            <BookOpen size={18} className="text-indigo-600" />
            <div className="font-black">Cómo funciona</div>
          </div>
          <div className="mt-6 space-y-4 text-slate-600 font-medium leading-relaxed">
            <div className="rounded-2xl border border-slate-100 bg-slate-50 px-6 py-5">
              Navegas pregunta a pregunta como en un test, pero con intención de aprendizaje.
            </div>
            <div className="rounded-2xl border border-slate-100 bg-slate-50 px-6 py-5">
              Al responder se marca la correcta y puedes desplegar la explicación.
            </div>
            <div className="rounded-2xl border border-slate-100 bg-slate-50 px-6 py-5">
