...
import {
  createHomeLoadTrace,
  markHomeHeroVisible,
  markHomeHeroFinalReady,
  markHomeLcpNodeMounted,
  markHomeCtaVisible,
  markHomeHydrated,
...
const loadTestSelection = () => import('../TestSelection');
const loadTestInterface = () => import('../TestInterface');
const loadStudyInterface = () => import('../StudyInterface');
const loadPostTestStats = () => import('../PostTestStats');
const loadStudyExplorer = () => import('../StudyExplorer');
const loadSettingsPanel = () => import('../SettingsPanel');
const loadStatsDashboard = () => import('../StatsDashboard');
const loadStudyQuestionBank = () => import('../StudyQuestionBank');
const loadTelemetryDebugPanel = () => import('../TelemetryDebugPanel');
const loadAdminQuestions = () => import('../admin/AdminQuestions');
const loadAdminDashboard = () => import('../admin/AdminDashboard');
const loadAdminStudents = () => import('../admin/AdminStudents');
const loadAdminCatalogs = () => import('../admin/AdminCatalogs');

const TestSelection = lazy(loadTestSelection);
const TestInterface = lazy(loadTestInterface);
const StudyInterface = lazy(loadStudyInterface);
const PostTestStats = lazy(loadPostTestStats);
const StudyExplorer = lazy(loadStudyExplorer);
const SettingsPanel = lazy(loadSettingsPanel);
const StatsDashboard = lazy(loadStatsDashboard);
const StudyQuestionBank = lazy(loadStudyQuestionBank);
const TelemetryDebugPanel = lazy(loadTelemetryDebugPanel);
const AdminQuestions = lazy(loadAdminQuestions);
const AdminDashboard = lazy(loadAdminDashboard);
const AdminStudents = lazy(loadAdminStudents);
const AdminCatalogs = lazy(loadAdminCatalogs);
import {
  buildCoachCopySeed,
...
export default function AuthenticatedAppShell({ session }: AuthenticatedAppShellProps) {
  ...
  const homeHeroFinalCycleRef = useRef<string | null>(null);
  const homeLcpNodeMarkedRef = useRef(false);
  const recentSessionsFetchInFlightRef = useRef(false);
  const [homeFirstPaintCache, setHomeFirstPaintCache] = useState<Record<string, HomeFirstPaintModel>>(
    () => readHomeFirstPaintCache(),
  );
  const [homeHeroFinalModel, setHomeHeroFinalModel] = useState<HomeHeroFinalModel | null>(null);
  ...
  useEffect(() => {
    ...
    let primaryHandle: IdleHandle | null = null;
    ...
    warmupTimer = setTimeout(scheduleWarmup, 3500);
  ...
  const refreshDashboard = useCallback(async () => {
    ...
    homeHeroMarkedRef.current = false;
    homeCtaMarkedRef.current = false;
    homeHeroFinalCycleRef.current = null;
    homeLcpNodeMarkedRef.current = false;
    setHomeHeroFinalModel(null);
    ...
    try {
      ...
      requestIdle(() => {
        void getRecentSessionsForHome(curriculum)
          .then((recentSessions) => {
            if (dashboardLoadCycleRef.current !== loadCycle) return;
            setBundle((prev) => {
              if (!prev) return prev;
        ...
        void (async () => {
          try {
            const nextBundle = await hydrateDashboardBundle(primaryBundle, curriculum, {
              skipSharedCommonOverride: true,
            });
            if (dashboardLoadCycleRef.current !== loadCycle) return;

            setBundle((prev) => {
              const recentSessions =
                prev?.practiceState.recentSessions?.length
                  ? prev.practiceState.recentSessions
                  : nextBundle.practiceState.recentSessions;
              return {
                ...nextBundle,
                practiceState: {
                  ...nextBundle.practiceState,
                  recentSessions,
                },
              };
            });
            ...
            requestIdle(() => {
              void (async () => {
                const sharedOverride = await computeSharedCommonOverride(curriculum).catch(() => null);
                if (!sharedOverride) return;
                if (dashboardLoadCycleRef.current !== loadCycle) return;
                setBundle((prev) => {
                  if (!prev?.practiceState.learningDashboardV2) return prev;
                  const learningDashboardV2 = prev.practiceState.learningDashboardV2;
                  return {
                    ...prev,
                    practiceState: {
                      ...prev.practiceState,
                      learningDashboardV2: {
                        ...
                        topicBreakdown: [
                          ...
                          ),
                          {
                            ...
                            attempts: sharedOverride.attempts,
                            questionCount: sharedOverride.total,
                            unseenCount: sharedOverride.unseen,
                            correctAttempts: 0,
                            accuracyRate: 0,
                          },
                        ],
  ...
  const refreshDashboardSilently = useCallback(async () => {
    ...
    try {
      const primaryBundle = await loadDashboardPrimaryBundle(curriculum);
      const nextBundle = await hydrateDashboardBundle(primaryBundle, curriculum, { skipSharedCommonOverride: true });
      setBundle((prev) => {
        const recentSessions =
          prev?.practiceState.recentSessions?.length ? prev.practiceState.recentSessions : nextBundle.practiceState.recentSessions;
        return {
          ...nextBundle,
          practiceState: {
            ...nextBundle.practiceState,
  ...
  useEffect(() => {
    ...
    if (homeHeroMarkedRef.current) return;
    const trace = homeLoadTraceRef.current;
    if (!trace) return;

    const frame = window.requestAnimationFrame(() => {
      markHomeHeroVisible(trace);
      homeHeroMarkedRef.current = true;
    });
  ...
  useEffect(() => {
    if (currentView !== 'dashboard') return;
    if (homeLcpNodeMarkedRef.current) return;
    const trace = homeLoadTraceRef.current;
    if (!trace) return;

    let rafId = 0;
    const check = () => {
      const node = document.querySelector('[data-home-lcp-title="true"]');
      if (node) {
        markHomeLcpNodeMounted(trace);
        homeLcpNodeMarkedRef.current = true;
        return;
      }
  ...
  }, [currentView, homeHeroFinalModel]);
  ...
  const homeFirstPaintModel = homeFirstPaintCache[curriculum] ?? null;

  useEffect(() => {
    if (!bundle) return;
    const nextModel: HomeFirstPaintModel = {
      coachLabel: heroCopy.eyebrow ?? t('Sugerencia de hoy', 'Gaurko proposamena'),
      coachTitle: heroCopy.line1,
      coachDescription: `${continuityLine ? `${continuityLine} ` : ''}${heroCopy.line2 ?? ''}`.trim(),
      coachCtaLabel: heroCopy.cta ?? t('Practicar ahora', 'Orain praktikatu'),
      primaryCardTitle: homeCardCopy.line1,
      primaryCardDescription: homeCardCopy.line2 ?? '',
      primaryCardCtaLabel: homeCardCopy.cta ?? t('Seguir por aqui', 'Hemendik jarraitu'),
    };

    setHomeFirstPaintCache((prev) => {
      const current = prev[curriculum];
      if (
        current &&
        current.coachLabel === nextModel.coachLabel &&
        current.coachTitle === nextModel.coachTitle &&
        current.coachDescription === nextModel.coachDescription &&
        current.coachCtaLabel === nextModel.coachCtaLabel &&
        current.primaryCardTitle === nextModel.primaryCardTitle &&
        current.primaryCardDescription === nextModel.primaryCardDescription &&
        current.primaryCardCtaLabel === nextModel.primaryCardCtaLabel
      ) {
        return prev;
      }
      const updated = { ...prev, [curriculum]: nextModel };
      try {
        window.localStorage.setItem(HOME_FIRST_PAINT_STORAGE_KEY, JSON.stringify(updated));
      } catch {
        // ignore
      }
      return updated;
  ...
  useEffect(() => {
    ...
    const trace = homeLoadTraceRef.current;
    if (!trace) return;
    if (homeHeroFinalCycleRef.current === trace.cycleId) return;

    const label = heroCopy.eyebrow ?? t('Sugerencia de hoy', 'Gaurko proposamena');
    const title = heroCopy.line1 || t('Preparando tu siguiente paso', 'Zure hurrengo pausoa prestatzen');
    const description = `${continuityLine ? `${continuityLine} ` : ''}${heroCopy.line2 ?? ''}`.trim();
    const ctaLabel = heroCopy.cta ?? t('Practicar ahora', 'Orain praktikatu');

    setHomeHeroFinalModel({
      label,
      title,
      description,
      ctaLabel,
    ...
    homeHeroFinalCycleRef.current = trace.cycleId;
    const frame = window.requestAnimationFrame(() => markHomeHeroFinalReady(trace));
    return () => window.cancelAnimationFrame(frame);
  }, [bundle, continuityLine, currentView, heroCopy.cta, heroCopy.eyebrow, heroCopy.line1, heroCopy.line2, t]);
  ...
  const handleRunFollowUp = useCallback(
    async (trigger: 'recommended' | 'clean' | 'errors') => {
      const { resolveFollowUpSession } = await import('../../lib/followUpSession');
  ...
  const handleStartCustomPractice = useCallback(
    async (config: CustomPracticeConfig) => {
      ...
      const { buildExecutableSessionPlanFromCustomPractice } = await import('../../lib/customPracticePlan');
  ...
  const gamification = useMemo(() => {
    ...
    try {
      ...
      if (recentSessions.length > 0) {
        ...
        const cached = Number(window.localStorage.getItem(cacheKey) ?? '0');
  ...
  useEffect(() => {
    ...
    if (!bundle) return;
    if (bundle.practiceState.recentSessions.length > 0) return;
    if (recentSessionsFetchInFlightRef.current) return;
    recentSessionsFetchInFlightRef.current = true;
    ...
    void getRecentSessionsForHome(curriculum)
      ...
      .finally(() => {
        recentSessionsFetchInFlightRef.current = false;
      });
  }, [bundle, curriculum, currentView]);
  ...
  const screenLoader = (
    <div className="flex min-h-[40vh] items-center justify-center py-16 text-slate-500">
      ...
      >
        ...
        {currentView === 'dashboard' ? (
          <Dashboard
            ...
            coachLabel={
              bundle
                ? homeHeroFinalModel?.label ?? heroCopy.eyebrow ?? t('Sugerencia de hoy', 'Gaurko proposamena')
                : homeFirstPaintModel?.coachLabel ?? t('Direccion inmediata', 'Berehalako norabidea')
            }
            coachTitle={
              bundle
                ? homeHeroFinalModel?.title ?? heroCopy.line1
                : homeFirstPaintModel?.coachTitle ?? t('Preparando tu siguiente paso', 'Zure hurrengo pausoa prestatzen')
            }
            coachDescription={
              bundle
                ? homeHeroFinalModel?.description ?? `${continuityLine ? `${continuityLine} ` : ''}${heroCopy.line2 ?? ''}`.trim()
                : homeFirstPaintModel?.coachDescription ??
                  t(
            ...
            coachCtaLabel={
              bundle
                ? homeHeroFinalModel?.ctaLabel ?? heroCopy.cta ?? t('Practicar ahora', 'Orain praktikatu')
                : homeFirstPaintModel?.coachCtaLabel ?? t('Preparando recomendacion', 'Gomendioa prestatzen')
            }
            ...
            primaryCardDescription={
              bundle
                ...
                : homeFirstPaintModel?.primaryCardDescription ??
                  t(
            ...
            primaryCardCtaLabel={
              bundle
                ? homeCardCopy.cta ?? t('Seguir por aqui', 'Hemendik jarraitu')
                : homeFirstPaintModel?.primaryCardCtaLabel ?? t('Preparando sesion', 'Saioa prestatzen')
            }
